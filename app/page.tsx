...
export default function HomePage() {
  ...
  const plans = [
    { name: 'Free', price: '$0', period: 'forever', features: ['5 active invoices', '3 follow-ups per invoice', 'Dashboard & stats', 'Mark as paid'], cta: 'Get started free', highlight: false },
    { name: 'Pro', price: '$9', period: 'per month', features: ['Unlimited invoices', 'Unlimited follow-ups', 'Custom email tone', 'Priority support', 'Everything in Free'], cta: 'Start free — upgrade anytime', highlight: true },
    { name: 'Agency', price: '$29', period: 'per month', features: ['Everything in Pro', 'Multiple team members', 'White-label emails', 'Client portal (soon)', 'Dedicated support'], cta: 'Get started free', highlight: false },
  ]
  ...
  return (
    <div style={{ fontFamily: 'DM Sans, system-ui, sans-serif', background: '#fff', color: '#1a1a18', overflowX: 'hidden' }}>
      ...
      <nav style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '1.1rem 2.5rem', background: 'rgba(255,255,255,0.92)', backdropFilter: 'blur(16px)', borderBottom: '0.5px solid rgba(0,0,0,0.08)' }}>
        <div style={{ fontFamily: 'Georgia, serif', fontStyle: 'italic', fontSize: 22, color: '#1a1a18' }}><span style={{ color: '#1a7a4a' }}>Pay</span>nelo</div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 28 }}>
          ...
          <button onClick={() => document.getElementById('get-started')?.scrollIntoView({ behavior: 'smooth' })} style={{ background: '#0f1a10', color: '#fff', border: 'none', borderRadius: 100, padding: '9px 22px', fontSize: 14, fontWeight: 500, cursor: 'pointer', fontFamily: 'inherit' }}>Get started free</button>
        </div>
      </nav>
      ...
      <section style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '9rem 2rem 5rem', textAlign: 'center', background: 'linear-gradient(180deg, rgba(200,240,117,0.07) 0%, #fff 70%)' }}>
        ...
        <h1 style={{ fontFamily: 'Georgia, serif', fontStyle: 'italic', fontWeight: 300, fontSize: 'clamp(3.2rem, 7vw, 6rem)', lineHeight: 1.02, letterSpacing: '-0.03em', color: '#1a1a18', maxWidth: 860, marginBottom: 28 }}>
          You did the work.<br /><span style={{ color: '#1a7a4a' }}>Now get paid for it.</span>
        </h1>
        ...
        <p style={{ fontSize: 13, color: '#1a7a4a', fontWeight: 500, marginBottom: 36 }}>Free to start &nbsp;·&nbsp; No awkward emails &nbsp;·&nbsp; Cancel anytime</p>
          ...
          <div style={{ background: '#fff', border: '0.5px solid rgba(0,0,0,0.08)', borderRadius: 20, padding: '2.5rem 2rem', boxShadow: '0 8px 40px rgba(0,0,0,0.07)' }}>
            ...
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              ...
              <button type="submit" disabled={loading}
                style={{ width: '100%', padding: '13px', background: loading ? '#f0efeb' : '#0f1a10', color: loading ? '#a8a8a2' : '#fff', border: 'none', borderRadius: 100, fontSize: 15, fontWeight: 500, cursor: 'pointer', fontFamily: 'inherit', marginTop: 4 }}>
            ...
            <div style={{ marginTop: 16, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              {!isSignUp && (
                <button onClick={() => router.push('/forgot-password')}
                  style={{ background: 'none', border: 'none', color: '#a8a8a2', cursor: 'pointer', fontSize: 13, fontFamily: 'inherit', textDecoration: 'underline' }}>
                  Forgot password?
                </button>
              )}
            </div>
      ...
      <section style={{ padding: '6rem 2rem', background: '#0f1a10' }}>
        <div style={{ maxWidth: 760, margin: '0 auto', textAlign: 'center' }}>
          ...
          <div style={{ padding: '1.5rem 2rem', background: 'rgba(200,240,117,0.08)', border: '0.5px solid rgba(200,240,117,0.2)', borderRadius: 14 }}>
            <p style={{ fontSize: 16, color: '#c8f075', fontWeight: 300, fontStyle: 'italic', fontFamily: 'Georgia, serif' }}>Paynelo handles all of this. So you never have to feel that way again.</p>
          </div>
        </div>
      </section>
      ...
      <section id="how" style={{ padding: '6rem 2rem', background: '#f7f7f5' }}>
        <div style={{ maxWidth: 900, margin: '0 auto', textAlign: 'center' }}>
          <h2 style={{ fontFamily: 'Georgia, serif', fontStyle: 'italic', fontWeight: 300, fontSize: 'clamp(2rem, 4vw, 3rem)', color: '#1a1a18', letterSpacing: '-0.03em', marginBottom: 12 }}>Set it up in 2 minutes</h2>
      ...
      <section id="pricing" style={{ padding: '6rem 2rem', background: '#fff' }}>
        <div style={{ maxWidth: 960, margin: '0 auto', textAlign: 'center' }}>
          <h2 style={{ fontFamily: 'Georgia, serif', fontStyle: 'italic', fontWeight: 300, fontSize: 'clamp(2rem, 4vw, 3rem)', color: '#1a1a18', letterSpacing: '-0.03em', marginBottom: 12 }}>Pricing that makes sense</h2>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: 20, alignItems: 'start' }}>
            {plans.map(plan => (
              <div key={plan.name} style={{ background: plan.highlight ? '#0f1a10' : '#f7f7f5', border: plan.highlight ? 'none' : '0.5px solid rgba(0,0,0,0.08)', borderRadius: 20, padding: '2.5rem', textAlign: 'left', position: 'relative', boxShadow: plan.highlight ? '0 8px 40px rgba(15,26,16,0.2)' : 'none' }}>
                {plan.highlight && <div style={{ position: 'absolute', top: -12, left: '50%', transform: 'translateX(-50%)', background: '#c8f075', color: '#0f1a10', fontSize: 11, fontWeight: 700, padding: '4px 16px', borderRadius: 100, whiteSpace: 'nowrap' }}>MOST POPULAR</div>}
      ...
      <footer style={{ padding: '2rem 2.5rem', borderTop: '0.5px solid rgba(0,0,0,0.08)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
        <div style={{ fontFamily: 'Georgia, serif', fontStyle: 'italic', fontSize: 20, color: '#1a1a18' }}><span style={{ color: '#1a7a4a' }}>Pay</span>nelo</div>
